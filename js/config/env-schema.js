/**
 * js/config/env-schema.js
 * ArqVértice Studio — Validador de Variáveis de Ambiente e Gestão de Secrets (Fail-Fast).
 */

(function (root, factory) {
  if (typeof module === 'object' && module.exports) module.exports = factory();
  else root.ArqVerticeEnvValidator = factory();
}(typeof self !== 'undefined' ? self : this, function () {
  'use strict';

  class EnvValidationError extends Error {
    constructor(missingFields, invalidFields) {
  ...
  function validateEnvironment(env = process.env) {
    ...
    if (!databaseUrl && isProdOrStaging) {
    ...
    if (!jwtSecret && isProdOrStaging) {
      missing.push('JWT_SECRET');
    } else if (jwtSecret && isProdOrStaging && jwtSecret.length < 32) {
      invalid.push({ field: 'JWT_SECRET', message: 'JWT_SECRET em staging/produção deve ter pelo menos 32 caracteres' });
    }

    const sessionSecret = env.SESSION_SECRET;
    if (!sessionSecret && isProdOrStaging) {
      missing.push('SESSION_SECRET');
    } else if (sessionSecret && isProdOrStaging && sessionSecret.length < 32) {
